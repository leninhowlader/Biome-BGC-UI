...
class stat:
    fun_list = ['Root Mean Square Error', 'Coefficient of Determination', 'Mean Absolute Percentage Error',
    ...
    def objective_function(fun_name, sim, obs, normalize_by_obs_max=False, normalize_by_obs_mean=False):
        ...
        elif fun_name == 'Mean Absolute Percentage Error':
            result = stat.mean_absolute_percentage_error(sim, obs)
    ...
    def pairing_variables(first_value_list, first_key_list, second_value_list, second_key_list):
        ...
        while i < len(a) and j < len(p):
            if a[i] == p[j]:
                if b[i] is not None and q[j] is not None:
    ...
    def data_evaluation(bgc_directory, bgc_init_filename, comparison_list, normby_obsmax=False, normby_obsmean=False,
        ...
        for comp_var in comparison_list:
            if comp_var.group_name not in group_list:
                group_list.append(comp_var.group_name)
        ...
        try:
            if len(comparison_list) > 0:
                ...
                for comp_var in comparison_list:
                    ...
                    bgc_output = model_output_dic[comp_var.model_file_type]
                    observed_dataset = observ_data_dic[ob_file_list.index(comp_var.observation_filename)]

                    #step-2: from collection (of data) select data for required variables
                    sim, obs = stat.comparison_specific_data_processing(comp_var, bgc_output, observed_dataset)
                    #step-3: calculate output of objective function

                    if sim and obs:
                        r = stat.objective_function(comp_var.objective_function, sim, obs, normby_obsmax, normby_obsmean)
                        if not (abs(r) > 1e10): #very big number resulted by division by very small numbers
                            if comp_var.weighing_factor not in [0, 1]: r *= comp_var.weighing_factor

                            ndx = group_list.index(comp_var.group_name)
        ...
        for i in range(len(success_count)):
            if success_count[i] == 0: group_result[i] = 'DNE'
    ...
    def collection_of_data(bgc_directory, bgc_init_filename, comparison_list):
        ...
        for comp_var in comparison_list:
            file_type = comp_var.model_file_type
            observ_dfile = comp_var.observation_filename
    ...
    @staticmethod
    def comparison_specific_data_processing(comp_var, bgc_output, observed_data):
        sim, obs, psim, pobs = [], [], [], []

        #step-1: from dataset read specific columns (variables); some variable needs specific pre-processing steps
        if comp_var.pairing_model_variable: psim = ReadBinaryOutput.ExtractColumnRecord(comp_var.pairing_model_variable, bgc_output.header_variable, bgc_output.record_list)
        if comp_var.pairing_observation_variable: pobs = ReadBinaryOutput.ExtractColumnRecord(comp_var.pairing_observation_variable, observed_data.header_variable, observed_data.record_list)

        if comp_var.model_variable_name in ['cf_site_hr_rr']:
            ...
            obs = ReadBinaryOutput.ExtractColumnRecord(comp_var.observation_variable_name, observed_data.header_variable, observed_data.record_list)
            comp_var.pairing_flag = True
        elif comp_var.model_variable_name in ['epv_vegt_proj_lai']:
            sim = ReadBinaryOutput.ExtractColumnRecord(comp_var.model_variable_name, bgc_output.header_variable, bgc_output.record_list)
            obs = ReadBinaryOutput.ExtractColumnRecord(comp_var.observation_variable_name, observed_data.header_variable, observed_data.record_list)
        ...
        elif comp_var.model_file_type.lower().find('monavg') >= 0 and comp_var.observation_variable_name.lower().find('cum') >= 0:
            sim = ReadBinaryOutput.ExtractColumnRecord(comp_var.model_variable_name, bgc_output.header_variable, bgc_output.record_list)
            obs = ReadBinaryOutput.ExtractColumnRecord(comp_var.observation_variable_name, observed_data.header_variable, observed_data.record_list)
        ...
        pair_flag = comp_var.pairing_flag
                ...
                for comp_var in comparison_list:
                    ...
                    for fun in stat.fun_list:
                        ...
                        try:
                            ...
                            if comp_var.weighing_factor not in [0, 1]: f *= comp_var.weighing_factor
                            if f == 'DNE': f = 'None'
