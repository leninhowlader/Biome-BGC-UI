...
import sys, os
sys.path.append('..')
from hopspack.configure import Solution, Configure, TargetParameter, Comparing_Variable
...
def make_graph(model_directory, starting_init_file, optimized_init_file, measured_data_source, save_flag=False, target_directory='', filename_prefix=''):
    ...
    graph_list_type2 = ['stand_precipitation_R2', 'canopy_evaporation_R2', 'soil_temperature_R2', 'soil_moisture_R2', 'leaf_area_index_R2',
...
def  main (argv):
    if len(argv) < 4:
        print('usages: solution_processing.py <plot no.> <objective function> <log directory>')
        exit(-1)
    ...
    log_file = os.path.join(log_dir, plot_no + '_' + obj_fun + '.log')
    initial_run = False
    database_flag = False
    # if argv[4].lower() == 'true': database_flag = True

    #step-1: reading configuration (and saving calibration info)
    #step-1.1: read configuration file
    config_file = os.path.join('config',plot_no, 'configuration_' + plot_no + '_' + obj_fun + '.txt' )
    ...
    #step-1.2: read no. of iteration and success rate
    iteration, failed, unsuccessful, success_rate = Solution.inspect_logfile(log_file)
    if iteration <= 0: iteration, success_rate = -1, 0
    ...
    param_opt_val = []
    solution_file = os.path.join('output', 'cal_best_point', plot_no + '_' + obj_fun + '.txt')
    if solution_file: param_opt_val = Solution.read_solution_file(solution_file)
    ...
    #step-6.1: read comparison policy file
    comparison_list = []
    ...
    config.stat_filename = 'statistics_all_plot.txt'
    # step-7.1: calculate statistics for initial condition and save results
    if initial_run:
        fun_list, result = [], []
        try:
            fun_list, result = stat.calculate_statistics_all(model_directory, initial_filename, comparison_list)
        except Exception as ex: pass

        write_stat_result_into_file(plot_no, config.objective_function, normalization_type, comparison_list, fun_list, result, config.stat_filename, is_init=True)

    #step-7.2: calculate statistics for optimized condition and save results
    fun_list, result = [], []
    try:
        fun_list, result = stat.calculate_statistics_all(model_directory, initial_filename_optimized, comparison_list)
    except Exception as ex:
        pass

    write_stat_result_into_file(plot_no, config.objective_function, normalization_type, comparison_list, fun_list, result, config.stat_filename, is_init=False)
